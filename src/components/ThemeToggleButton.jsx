  ...
  return (
    <button
      className={`w-12 h-6 rounded-full relative ${
        theme === 'dark' ? 'bg-gray-800' : 'bg-[#b4e8fa]'
      } flex items-center transition duration-300 focus:outline-none shadow`}
      onClick={handleToggle}
    >
      <img
        className='absolute cursor-pointer w-full h-full object-cover inset-0 rounded-3xl'
      ...
      <div
        className={`w-6 h-6 relative rounded-full transition duration-500 transform p-1 text-white z-10 ${
          theme === 'dark' ? 'bg-gray-700 translate-x-6' : 'bg-yellow-500 -translate-x-1'
        }`}
      >
