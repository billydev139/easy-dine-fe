...
import EditCashReceipt from './edit-cash-receipt';
...
const InvoiceGenerator = () => {
  ...
  const renderInvoiceDetails = () => {
    const leftColumn = (
      <div className='text-sm'>
        <div className='font-semibold'>Rechnung {invoiceData.invoiceNumber}</div>
        <div className='grid grid-cols-2 gap-x-2'>
          <div>Rechnunddatum:</div>
          <div>{invoiceData.invoiceDate}</div>
          <div>Fällig am:</div>
          <div>{invoiceData.orderDate}</div>
          <div>Annahme:</div>
          <div>{invoiceData.orderTime}</div>
          <div>Ihre UID:</div>
          <div>{invoiceData.ihreUID}</div>
          <div>Zahlungsart:</div>
          <div>{invoiceData.paymentType}</div>
        </div>
      </div>
    );
    ...
    const rightColumn = (
      <div className='text-sm'>
        <div className='grid grid-cols-2 gap-x-2'>
          <div>Kd.-Nr:</div>
          <div>{invoiceData.customerID}</div>
          <div></div>
    ...
    return (
      <div className='border border-dashed border-[#0075FF] rounded-sm mt-4'>
        ...
        <div className='flex justify-between border-b border-dashed border-gray-300 p-3'>
          {leftColumn}
          {rightColumn}
        </div>
      </div>
    ...
    return (
      <div className='border border-dashed border-[#0075FF] rounded-sm mt-4'>
        ...
        <div className='border-b border-dashed border-gray-300 p-4'>
          {layoutOptions.textAboveItems && (
            <div className='mb-4 text-sm'>{layoutOptions.textAboveItems}</div>
          )}
    ...
    <div className='min-h-screen'>
      <div className=''>
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
          <div>
            <h1 className='text-2xl font-bold mb-4'>Layout Invoice</h1>
